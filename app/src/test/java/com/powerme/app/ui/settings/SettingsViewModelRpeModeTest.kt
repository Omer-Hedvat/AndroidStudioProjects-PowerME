...
class SettingsViewModelRpeModeTest {
    ...
    @Test
    fun `init - rpeMode defaults to OFF from DataStore`() = runTest(testDispatcher) {
        val viewModel = buildViewModel()
        runCurrent()

        assertEquals(RpeMode.OFF, viewModel.uiState.value.rpeMode)
    }

    @Test
    fun `init - rpeMode reflects DataStore value HYBRID`() = runTest(testDispatcher) {
        whenever(mockAppSettingsDataStore.rpeMode).thenReturn(flowOf(RpeMode.HYBRID))
        val viewModel = buildViewModel()
        runCurrent()

        assertEquals(RpeMode.HYBRID, viewModel.uiState.value.rpeMode)
    }

    @Test
    fun `init - rpeMode reflects DataStore value PURE_GYM`() = runTest(testDispatcher) {
        whenever(mockAppSettingsDataStore.rpeMode).thenReturn(flowOf(RpeMode.PURE_GYM))
        val viewModel = buildViewModel()
        runCurrent()

        assertEquals(RpeMode.PURE_GYM, viewModel.uiState.value.rpeMode)
    }

    @Test
    fun `init - rpeMode reflects DataStore value PURE_FUNCTIONAL`() = runTest(testDispatcher) {
        whenever(mockAppSettingsDataStore.rpeMode).thenReturn(flowOf(RpeMode.PURE_FUNCTIONAL))
        val viewModel = buildViewModel()
        runCurrent()

        assertEquals(RpeMode.PURE_FUNCTIONAL, viewModel.uiState.value.rpeMode)
    }

    @Test
    fun `setRpeMode - writes to DataStore and updates uiState`() = runTest(testDispatcher) {
        val viewModel = buildViewModel()
        runCurrent()

        viewModel.setRpeMode(RpeMode.HYBRID)
        runCurrent()

        verify(mockAppSettingsDataStore).saveRpeMode(RpeMode.HYBRID)
        assertEquals(RpeMode.HYBRID, viewModel.uiState.value.rpeMode)
    }

    @Test
    fun `setRpeMode - pushes app preferences to Firestore`() = runTest(testDispatcher) {
        val viewModel = buildViewModel()
        runCurrent()

        viewModel.setRpeMode(RpeMode.PURE_GYM)
        runCurrent()
    ...
    @Test
    fun `setRpeMode - switching to PURE_FUNCTIONAL updates uiState`() = runTest(testDispatcher) {
        val viewModel = buildViewModel()
        runCurrent()

        viewModel.setRpeMode(RpeMode.PURE_FUNCTIONAL)
        runCurrent()

        verify(mockAppSettingsDataStore).saveRpeMode(RpeMode.PURE_FUNCTIONAL)
        assertEquals(RpeMode.PURE_FUNCTIONAL, viewModel.uiState.value.rpeMode)
    }
}
