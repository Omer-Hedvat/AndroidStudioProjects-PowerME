...
class HealthConnectManager @Inject constructor(
    ...
    /**
     * Read exercise sessions from Health Connect (last 30 days).
     * Returns empty list if unavailable, not granted, or on any failure.
     */
    suspend fun getExerciseSessions(): List<ExerciseSessionRecord> = withContext(Dispatchers.IO) {
        try {
            if (!isAvailable()) return@withContext emptyList()
            val client = HealthConnectClient.getOrCreate(context)
            if (!hasPermission(client, ExerciseSessionRecord::class)) return@withContext emptyList()
            val end = Instant.now()
            val start = end.minus(30, ChronoUnit.DAYS)
            val response = client.readRecords(
                ReadRecordsRequest(
