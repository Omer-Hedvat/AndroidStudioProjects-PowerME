...
- User profile (dateOfBirth, height, weight, gender, goals, chronotype, occupation); **Profile Setup flow** uses a two-step screen: Step 1 = Health Connect offer (HC permission launcher via `PermissionController.createRequestPermissionResultContract()`; skipped if HC unavailable; auto-advances if already granted), Step 2 = profile form with Google `displayName` pre-fill + HC body-metrics pre-fill (weight/height/bodyFat show "from Health Connect" badge); DOB uses M3 `DatePickerDialog`; **unit system selector (Metric/Imperial) at top of Step 2** — `SingleChoiceSegmentedButtonRow` persists via `ProfileSetupViewModel.setUnitSystem()` → `AppSettingsDataStore`; height/weight display fields convert on unit switch using internal metric ground-truth (`heightCmInternal`, `weightKgInternal`) + `LaunchedEffect(unitSystem)`; `ProfileSetupViewModel` (`ui/auth/ProfileSetupViewModel.kt`) injects `HealthConnectManager`, `UserSessionManager`, `FirebaseAuth`, `AppSettingsDataStore`; `saveProfile()` moved from `AuthViewModel` to `ProfileSetupViewModel`
...
**Unit Test Coverage (src/test/, 18 files, 325 tests total — all passing):**
