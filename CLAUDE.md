...
**Exercise.tags (P8 Tier 0, in-progress):** `tags TEXT NOT NULL DEFAULT '[]'` JSON column on `exercises` table (v50). `MasterExerciseSeeder` bumped to v2.1 — seeds 25 new functional movements (Olympic lifts, MetCon staples, Gymnastics, Monostructural cardio) and adds tags to 27 existing exercises. Tags: `"functional"`, `"functional-only"`, `"olympic"`, `"gymnastics"`, `"monostructural"`. `ExercisesViewModel` gains `functionalFilter: Boolean` in `ExercisesUiState` + `onFunctionalFilterToggled()`. `ExercisesScreen` has a "Functional" `FilterChip` (Mode row, `TimerGreen` colour) below the Equipment row.
