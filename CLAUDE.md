...
- Database: Room (v36, 17 entities, 17 DAOs)
...
- **Organize Mode (Active Workout):** Persistent multi-action mode entered from Management Hub → "Organize Exercises". Users can form multiple supersets, break supersets, and drag-reorder any number of exercises in one session. CAB shows Done (left) + "Organize • N selected" (center) + contextual icon (right): **Group** (Sync, enabled ≥2) when selection is not a single superset; **Ungroup** (LinkOff, always enabled) when all selected exercises share the same `supersetGroupId`. `commitSupersetSelection()` groups selection; `ungroupSelectedExercises()` dissolves selected exercises from their superset (auto-dissolves group if fewer than 2 would remain). Neither auto-exits — only the Done button calls `exitSupersetSelectMode()`. No `isReorderMode` flag exists on `ActiveWorkoutState`; drag handles on regular `ExerciseCard` rows remain always-visible for quick one-off reorders outside the mode.
...
- Unit system: `UnitSystem` enum (METRIC/IMPERIAL) in `data/UnitSystem.kt`; `AppSettingsDataStore.unitSystem: Flow<UnitSystem>` + `setUnitSystem()`; `UnitConverter` stateless object in `util/UnitConverter.kt` — single source of truth for all weight/height/distance conversions and formatted display strings; **storage always in metric (kg/cm/km), conversion at display/input boundary only — no DB migration needed**; BarType gains `weightLbs: Double` (gym-standard values) + `displayWeight(unit)`; PlateCalculator unit-aware (METRIC_DEFAULT_PLATES / IMPERIAL_DEFAULT_PLATES, unit-aware error threshold); all relevant ViewModels expose `unitSystem: StateFlow<UnitSystem>`; SettingsScreen "Units" card (SingleChoiceSegmentedButtonRow Metric/Imperial), unit-aware height input (ft+in for IMPERIAL), unit-aware weight/height labels; BodyVitalsCard, ProfileSetupScreen, ActiveWorkoutScreen (post-workout summary, DIST header), HistoryScreen, WorkoutDetailScreen all unit-aware
...
- User profile (dateOfBirth, height, weight, gender, goals, chronotype, occupation); **Profile Setup flow** — two-step onboarding screen (HC offer → profile form). See **PROFILE_SETUP_SPEC.md** for full details. All profile fields (Name, DOB, gender, occupation, chronotype, sleep, children, training targets) are also editable post-onboarding from the Settings "Personal Info" card. See **SETTINGS_SPEC.md**.
...
- Clocks tab (Stopwatch, Timer, Tabata, EMOM) with countdown beeps, skip-last-rest, pre-finish alerts, input validation; TABATA and EMOM use persistent side-by-side Start+Reset buttons (always visible, PlayArrow/Pause icons); STOPWATCH and COUNTDOWN use icon-labeled toggle+reset layout; centisecond display (`.cc` suffix, 28sp Normal, 0.55α) shown while actively running — counts 00→99 for Stopwatch, 99→00 for countdown modes, derived from `tickEpochMs` wall-clock epoch stamped each 1s tick; hidden on pause/idle/setup; progress bar smoothly interpolates within each second; Tabata toggle label is "Skip last rest" with `onSurface` thumb color (white in dark theme, black in light theme)
...
- v31 UUID String PKs + Firestore Sync: MIGRATION_30_31 — Workout/WorkoutSet/Routine/RoutineExercise PKs changed from Long→String (UUID); `WarmupLog.workoutId` FK also becomes String; create-copy-drop-rename migration preserves existing rows with string-cast IDs; `+updatedAt: Long` and `+isArchived: Boolean` on Workout/Routine; `AND isArchived = 0` filters in all list queries; new `data/sync/FirestoreSyncManager.kt` with fire-and-forget push (no `.await()`) and LWW pull; `finishWorkout()` + `saveRoutineEdits()` + `archiveRoutine()` + `deleteRoutine()` call push; Settings screen gains "Restore from Cloud" card with spinner + auth guard; soft deletes: `isArchived=true` instead of hard delete for workouts and routines. **FirestoreSyncManager extended (no DB bump):** `pushSettings()` + `pullSettings()` (LWW for `UserSettings` entity — plates, timers, haptics); `pushAppPreferences()` + `pullAppPreferences()` (DataStore prefs: themeMode, unitSystem, dailyStepTarget, keepScreenOn, language, enrichmentModel); `pullFromCloud()` now parallelizes 5 fetches (profile + settings + appPreferences + workouts + routines); `SyncResult` gains `settingsImported: Boolean`; `DatabaseModule.provideFirestoreSyncManager()` injects `UserSettingsDao` + `AppSettingsDataStore`. **SettingsViewModel push wiring:** `setThemeMode()`, `setUnitSystem()`, `toggleKeepScreenOn()` call `pushAppPreferences()` fire-and-forget; plate/audio/haptics toggle calls `pushSettings()` with `updatedAt` timestamp. `AuthViewModel` fix: `setHasRestoredOnce(true)` moved to **after** `pullProfileOnly()` so a failed pull doesn't permanently skip future restores. `backup_rules.xml` + `data_extraction_rules.xml`: excluded `datastore/app_settings.preferences_pb` from Android backup so `has_restored_once` gate resets on fresh install.
...
- v34 Date of Birth: MIGRATION_33_34 — adds `dateOfBirth INTEGER DEFAULT NULL` to `users`; legacy `age` column preserved; `User.ageYears: Int?` extension computes from `dateOfBirth` or falls back to `age`; `WarmupService` + `MetricsViewModel` updated to use `ageYears`
- v35 Sync Foundation: MIGRATION_34_35 — adds `syncId TEXT NOT NULL DEFAULT ''` (UUID, stable cross-device Firestore identity) to: `exercises`, `exercise_muscle_groups`, `medical_ledger`, `metric_log`, `gym_profiles`, `warmup_log`, `chat_messages`, `warmup_library`, `state_history`; adds `updatedAt INTEGER NOT NULL DEFAULT 0` (LWW conflict resolution epoch ms) to: `exercises`, `medical_ledger`, `gym_profiles`, `user_settings`, `users`; existing rows backfilled with SQLite UUID v4 via `randomblob`; DB version bumped 34→35; all entity data classes updated with `@ColumnInfo(defaultValue = ...)` annotations
- v36 Routine Name Denormalization: MIGRATION_35_36 — adds `routineName TEXT DEFAULT NULL` to `workouts`; back-fills from `routines.name` where `routineId IS NOT NULL`; purpose: History cards retain the routine name even after the routine is deleted (`onDelete = SET_NULL` nullifies `routineId` but `routineName` snapshot persists); `WorkoutDao.getAllCompletedWorkoutsWithExerciseNames` uses `COALESCE(w.routineName, r.name)`; `WorkoutRepository.instantiateWorkoutFromRoutine()` stamps name at creation; `WorkoutViewModel.finishWorkout()` writes `routineName` on the saved entity; `WorkoutViewModel.rehydrateIfNeeded()` prefers `activeWorkout.routineName` over a live DAO lookup
...
- **Superset UI fixes** (no DB bump): (1) `WorkoutDetailScreen` superset spine — was hardcoded to `MaterialTheme.colorScheme.secondary` (single purple for all groups); fixed to use `supersetColor(supersetId)` from `Color.kt` so distinct groups show distinct palette colors; (2) `RoutineOverviewSheet` superset lines — added 4dp colored `Box` on left edge of each exercise row when `supersetGroupId != null`, matching Active Workout visual pattern; 8dp start padding separates bar from text; (3) `WorkoutDetailScreen` exercise ordering — `getSetsWithExerciseForWorkout` now uses `ORDER BY ws.rowid ASC, ws.setOrder ASC` instead of `ORDER BY ws.exerciseId, ws.setOrder`; SQLite `rowid` auto-increments on insertion so exercises appear in original workout order without a schema change
...
- `ui/workout/WorkoutViewModelTest.kt` — 58 tests (includes 2 completeSet toggle + 5 rest timer/override + 5 per-set-type rest + 5 cascade/routine-sync + 1 selectSetType + 2 deleteSet timer cancel + 2 deleteRestSeparator + 1 updateExerciseRestTimers clears hidden separators + 4 edit mode + 1 helper + 2 Step E collapse/reorder + 1 Step F STRUCTURE sync + 9 Organize Mode + 3 Organize Mode ungroup + 1 routineName denormalization)
...
- `ui/metrics/MetricsViewModelBodyVitalsTest.kt` — 6 tests (HC state machine, BMI, 7d delta, sync success/failure)
- `ui/auth/AuthViewModelGoogleSignInTest.kt` — 9 tests (cancellation silent, no-credential error, generic error, new-user needsProfileSetup, returning-user isSignedIn, collision → pendingLinkEmail set, dismissLinkPrompt → cleared, auto-restore triggers when hasRestoredOnce=false, auto-restore skipped when hasRestoredOnce=true)
- `ui/auth/ProfileSetupViewModelTest.kt` — 7 tests (HC unavailable, HC available not-granted, HC already-granted auto-read, permission-result granted, permission-result denied, HC null data, skipHc)
