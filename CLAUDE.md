...
- **Health Connect Body & Vitals card** (Trends tab): `BodyVitalsCard` composable renders at top of `MetricsScreen`; 4×3 grid — Row 1: Age|Weight|BMI, Row 2: Body Fat|Height|Steps, Row 3: Sleep|HRV|RHR, Row 4: Lean Mass|Bone Mass|BMR (optional, from smart scales via HC); four states: CHECKING / UNAVAILABLE / AVAILABLE_NOT_GRANTED / AVAILABLE_GRANTED; inline sync button; "Connect" CTA navigates to Settings. `HealthConnectManager` has 10 total READ permissions: 7 CORE_PERMISSIONS (required for "connected" state) + 3 BODY_COMPOSITION_PERMISSIONS (optional, graceful degradation to "--"). `syncAndRead()` auto-persists weight/bodyFat/height/BMR/boneMass/leanBodyMass to `metric_log` and `users`. `MetricTile` has `deltaPositiveIsGood` param (Lean Mass uses `true` — gaining is good). `MetricsViewModel` injects HC manager + sync DAO + user session + AppSettingsDataStore.
...
**Unit Test Coverage (src/test/, 20 files, ~351 tests total — all passing):**
