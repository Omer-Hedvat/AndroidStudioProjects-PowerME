...
| [BUG_body_composition_ignores_hc](BUG_body_composition_ignores_hc.md) | BodyCompositionCard shows empty state — does not pull weight/body fat from Health Connect | 🔴 Blocked (external HC issue) | P1 | — | — | `HealthConnectManager.kt`, `TrendsRepository.kt`, `TrendsRepositoryBodyCompositionTest.kt` (new), `TrendsRepositoryChronotypeTest.kt`, `TrendsRepositoryWeeklyGroupingTest.kt` |
| [BUG_training_window_no_axis_labels](BUG_training_window_no_axis_labels.md) | Training Window scatter plot missing axis labels (X = hour, Y = volume) | ✅ Wrapped | P2 | — | — | `ChronotypeCard.kt`, `MetricsScreen.kt` |
| [BUG_hc_extended_reads_no_data](BUG_hc_extended_reads_no_data.md) | All HC data missing — full Health Connect sync broken (weight, body fat, HR, VO₂, SpO₂, Calories) | 🔴 Blocked (external HC issue) | P0 | — | — | `DatabaseModule.kt`, `PowerMeDatabase.kt`, `HealthConnectSync.kt`, `HealthConnectManager.kt`, `MetricLog.kt`, `HealthConnectSyncDao.kt`, `AndroidManifest.xml`, `MetricsViewModel.kt`, `BodyVitalsCard.kt`, `ReadinessEngine.kt`, `TrendsDao.kt`, `TrendsModels.kt`, `TrendsRepository.kt`, `TrendsViewModel.kt`, `PowerMeApplication.kt` + 7 new source files + 9 new test files |
...
| [BUG_exercise_detail_scroll_crash](BUG_exercise_detail_scroll_crash.md) | Exercise Detail Screen crashes on scroll | ✅ Wrapped | P1 | — | Exercise Detail Tabs v2 ✅ | `ExerciseDetailScreen.kt` |
...
| [BUG_warmup_ramp_format_crash](BUG_warmup_ramp_format_crash.md) | WarmUpRampSection crashes with UnknownFormatConversionException when percentageLabel contains % | ✅ Wrapped | P0 | — | — | `AboutTab.kt` |
...
| [BUG_warmup_sets_staggered_collapse](BUG_warmup_sets_staggered_collapse.md) | Warmup sets collapse simultaneously with rest separator instead of staggered | ✅ Wrapped | P2 | — | — | `WorkoutViewModel.kt`, `ActiveWorkoutScreen.kt`, `WorkoutViewModelTest.kt` |
...
| [BUG_rest_timer_skip_label](BUG_rest_timer_skip_label.md) | Rest timer TimerControlsSheet skip button shows checkmark icon instead of "Next" label | ✅ Wrapped | P3 | — | — | `ActiveWorkoutScreen.kt` |
...
| [BUG_save_routine_missing_workouts_tab](BUG_save_routine_missing_workouts_tab.md) | Saved routine missing from Workouts tab after post-workout Save Routine flow | 🔵 Open | P1 | — | — | — |
