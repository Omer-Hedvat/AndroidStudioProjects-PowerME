...
10. **Edit mode is strictly modal (with minimization exception):** While `isEditMode=true`, all explicit exit paths inside `ActiveWorkoutScreen` (back press, ✕ cancel icon) must show a [Discard Edits] / [Keep Editing] confirmation dialog. The ↓ chevron and bottom nav taps are the sole silent exit — they minimize (not cancel) edit mode. Never silently discard edits without user confirmation.
